...
- 증정품 기준을 충족하고 증정품을 가져오지 않았으며 증정품 제공이 가능한 경우에 한해 혜택 안내메시지를 출력한다.
    - 에매한 예제 케이스 모음
        - 2+1 프로모션 재고가 3개 남았고 구매자가 1개를 가져온 경우, 안내 없이 바로 계산
        - 2+1 프로모션 재고가 3개 남았고 구매자가 2개를 가져온 경우, 무료 증정 안내
        - 2+1 프로모션 재고가 2개 남았고 구매자가 2개를 가져온 경우 안내 없이 바로 계산
        - 2+1 프로모션 재고가 2개 남았고 구매자가 3개를 가져온 경우,  `1개는 프로모션 할인이 적용되지 않습니다` 문구 출력
...
5. 멤버십 할인 여부 입력받기
    - 프로모션 적용 후 남은 금액에 30% 할인 적용
        - (증정품가격+본품가격)을 공제하고 남은 금액을 의미한다.
    - 멤버십 할인이 불가능한 상황에도 일단 물어는 본다.
    - 영수증당 8000원으로 할인을 제한한다.
6. 최종 결제 금액 계산하기
    - 총구매액, 행사할인, 멤버십할인을 적용하여 내실돈을 계산한다
7. 구매 상품 내역, 증정 상품 내역, 금액 정보를 담은 영수증 발행하기
    - 기능 요구사항의 영수증 출력 파트 참조하여 출력
8. 추가 구매 여부를 입력받기
    - Y를 선택한 경우 업데이트된 재고와 함께 1번부터 새로 입력받기
    - N을 선택하여 종료
